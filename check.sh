...
python -m py_compile main.py keycan/__init__.py keycan/app.py keycan/window.py keycan/core/__init__.py keycan/core/typing_engine.py keycan/data/__init__.py keycan/data/database.py keycan/data/database_runtime.py keycan/gui/content_manager2.py keycan/gui/settings2.py keycan/gui/statistics_clean.py keycan/gui/search.py keycan/gui/workspace.py tests/test_stage1_database.py tests/test_stage3_database.py tests/test_custom_content.py
...
try:
    ...
    assert connection.execute("SELECT COUNT(*) FROM sources WHERE relative_path LIKE 'REVERSE ENGINEERING/%' OR relative_path LIKE 'REVERSE ENGINEERING\\\\%'").fetchone()[0] == 0
    ...
    assert {"lesson_id","duration_seconds","correct_words","wrong_words","words_per_minute","characters_per_minute","completed_at","source_name_snapshot","lesson_title_snapshot","target_word_count","typed_word_count","total_characters","correct_characters","wrong_characters","accuracy_percent"} <= result_columns
    ...
    assert connection.execute("SELECT COUNT(*) FROM sources WHERE is_custom=1 AND is_deleted=0 AND display_name=''").fetchone()[0] == 0
