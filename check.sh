...
python -m py_compile main.py keycan/__init__.py keycan/app.py keycan/window.py keycan/core/__init__.py keycan/core/typing_engine.py keycan/data/__init__.py keycan/data/database.py keycan/data/database_compat3.py keycan/gui/content_manager2.py keycan/gui/settings2.py keycan/gui/statistics_clean.py keycan/gui/search.py keycan/gui/workspace.py tests/test_stage1_database.py tests/test_stage3_database.py tests/test_custom_content.py
...
try:
    ...
    assert connection.execute("SELECT COUNT(*) FROM sources WHERE relative_path LIKE 'REVERSE ENGINEERING/%' OR relative_path LIKE 'REVERSE ENGINEERING\\\\%'").fetchone()[0] == 0, "Kaynak yolunda eski prefix kaldı"
    orphan_results = connection.execute("SELECT COUNT(*) FROM practice_results AS r LEFT JOIN lessons AS l ON l.id = r.lesson_id WHERE l.id IS NULL").fetchone()[0]
    ...
    required_result_columns = {"lesson_id", "duration_seconds", "correct_words", "wrong_words", "words_per_minute", "characters_per_minute", "completed_at", "source_name_snapshot", "lesson_title_snapshot", "target_word_count", "typed_word_count", "total_characters", "correct_characters", "wrong_characters", "accuracy_percent"}
    ...
    assert connection.execute("SELECT COUNT(*) FROM sources WHERE is_custom = 1 AND is_deleted = 0 AND display_name = ''").fetchone()[0] == 0, "Boş kullanıcı grubu bulundu"
    invalid_counts = connection.execute("SELECT COUNT(*) FROM practice_results WHERE duration_seconds < 0 OR correct_words < 0 OR wrong_words < 0 OR target_word_count < 0 OR typed_word_count < 0 OR total_characters < 0 OR correct_characters < 0 OR wrong_characters < 0 OR words_per_minute < 0 OR characters_per_minute < 0 OR accuracy_percent < 0 OR accuracy_percent > 100").fetchone()[0]
    assert invalid_counts == 0, "Sonuçlarda geçersiz çalışma değeri bulundu"
    inconsistent_character_counts = connection.execute("SELECT COUNT(*) FROM practice_results WHERE total_characters != correct_characters + wrong_characters").fetchone()[0]
