...
def _ensure_default_snapshot(self: Database) -> None:
    ...
    if self.conn.execute("SELECT COUNT(*) FROM default_content_sources").fetchone()[0]==0:
        for row in self.conn.execute("SELECT id,display_name,relative_path,custom_key FROM sources WHERE is_custom=0 ORDER BY id").fetchall(): self.conn.execute("INSERT INTO default_content_sources VALUES (?,?,?,?)",row)
        for row in self.conn.execute("SELECT id,source_id,legacy_metin_id,title,text,custom_order FROM lessons WHERE is_custom=0 ORDER BY source_id,legacy_metin_id,id").fetchall(): self.conn.execute("INSERT INTO default_content_lessons VALUES (?,?,?,?,?,?)",row)
        self.conn.commit()
...
def _move_lesson(self: Database,lesson_id: int,direction: int)->None:
    ...
    row=self.conn.execute("SELECT source_id FROM lessons WHERE id=? AND is_deleted=0",(lesson_id,)).fetchone()
    if not row: raise ValueError("Metin bulunamadı")
    source_id=int(row[0]); ids=[r[0] for r in self.conn.execute("SELECT id FROM lessons WHERE source_id=? AND is_deleted=0 ORDER BY custom_order,legacy_metin_id,id",(source_id,)).fetchall()]; index=ids.index(lesson_id); target=index+direction
    if target<0 or target>=len(ids): return
    ids[index],ids[target]=ids[target],ids[index]; self.conn.execute("BEGIN")
    try:
...
def _restore_defaults(self: Database):
    ...
    try:
        for source_id,name,path,key in source_rows: self.conn.execute("UPDATE sources SET display_name=?,relative_path=?,custom_key=?,is_custom=0,is_deleted=0 WHERE id=?",(name,path,key,source_id))
        ids={row[0] for row in source_rows}
        if ids: self.conn.execute(f"UPDATE lessons SET is_deleted=1 WHERE source_id IN ({','.join('?' for _ in ids)})",tuple(ids))
        for lesson_id,source_id,legacy_id,title,text,order in lesson_rows: self.conn.execute("UPDATE lessons SET source_id=?,legacy_metin_id=?,title=?,text=?,is_custom=0,is_deleted=0,custom_order=? WHERE id=?",(source_id,legacy_id,title,text,order,lesson_id))
        self.conn.commit()
    except Exception: self.conn.rollback(); raise
    ...
    try:
        for group in groups:
            ...
            for order,item in enumerate(group.get("lessons",[])):
                ...
                else: lesson_id=int(self.conn.execute("INSERT INTO lessons(source_id,legacy_metin_id,title,text,is_custom,is_deleted,custom_order,custom_key) VALUES(?,0,'Ders',?,?,0,?,?)",(source_id,text,int(is_custom),order,lesson_key)).lastrowid)
