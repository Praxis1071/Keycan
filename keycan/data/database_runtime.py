...
def _sources(self):
    rows=self.conn.execute("SELECT id,display_name,is_custom FROM sources WHERE is_deleted=0").fetchall(); visible=[]
    ...
    for source_id,name,is_custom in visible:
        ...
        else: builtin+=1; result.append((source_id,self._display_source_name(name,builtin)))
    return result
...
def _lessons(self,source_id):
    ...
    if row and row[0]:
        rows=self.conn.execute("SELECT id FROM lessons WHERE source_id=? AND is_custom=1 AND is_deleted=0 ORDER BY custom_order,id",(source_id,)); return [(lesson_id,str(i)) for i,(lesson_id,) in enumerate(rows.fetchall(),1)]
    rows=self.conn.execute("SELECT id FROM lessons WHERE source_id=? AND is_deleted=0 ORDER BY legacy_metin_id,id",(source_id,)).fetchall(); return [(lesson_id,f"Ders {i}") for i,(lesson_id,) in enumerate(rows,1)]
...
def _import_data(self,raw):
    ...
    try:
        for group in groups:
            ...
            if existing: source_id=int(existing[0]); self.conn.execute("UPDATE sources SET display_name=?,is_deleted=0 WHERE id=?",(name,source_id))
            ...
            for order,item in enumerate(lessons):
                ...
                if found: lesson_id=int(found[0]); self.conn.execute("UPDATE lessons SET source_id=?,text=?,title='Ders',custom_order=?,is_deleted=0 WHERE id=?",(source_id,text,order,lesson_id))
        ...
        existing={tuple(row) for row in self.conn.execute("SELECT completed_at,lesson_id,duration_seconds,words_per_minute,typed_word_count,accuracy_percent FROM practice_results WHERE completed_at!=''").fetchall()}; imported=skipped=0
        for item in results:
            if not isinstance(item,dict): skipped+=1; continue
            lesson_id=lesson_map.get(str(item.get("lesson_key","")).strip())
            if lesson_id is None:
                source_name=clean_source_name(str(item.get("source_name",""))); title=str(item.get("lesson_title","")); candidates=self.conn.execute("SELECT l.id,s.display_name,l.title FROM lessons l JOIN sources s ON s.id=l.source_id WHERE s.is_custom=0 AND s.is_deleted=0 AND l.is_deleted=0").fetchall()
...
def _reset_custom_content(self):
    groups=int(self.conn.execute("SELECT COUNT(*) FROM sources WHERE is_custom=1 AND is_deleted=0").fetchone()[0]); lessons=int(self.conn.execute("SELECT COUNT(*) FROM lessons WHERE is_custom=1 AND is_deleted=0").fetchone()[0])
    self.conn.execute("BEGIN")
    try:
        self.conn.execute("UPDATE sources SET is_deleted=1 WHERE is_custom=1 AND is_deleted=0")
        self.conn.execute("UPDATE lessons SET is_deleted=1 WHERE is_custom=1 AND is_deleted=0")
        self.conn.commit(); return groups,lessons
    except Exception: self.conn.rollback(); raise
...
Database.reset_custom_content=_reset_custom_content
