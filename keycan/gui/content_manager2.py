...
class ContentManagerWindow(Adw.Window):
    ...
    def _build(self):
        ...
        left=Gtk.Box(orientation=Gtk.Orientation.VERTICAL,spacing=8); left.set_margin_end(12); panes.set_start_child(left); label=Gtk.Label(label="Ders grupları"); label.set_xalign(0); label.add_css_class("title-3"); left.append(label)
        self.group_list=Gtk.ListBox(); self.group_list.set_selection_mode(Gtk.SelectionMode.SINGLE); self.group_list.set_show_separators(False); self.group_list.set_vexpand(True); self.group_list.connect("row-selected",self._group_selected); scroll=Gtk.ScrolledWindow(); scroll.set_vexpand(True); scroll.set_child(self.group_list); left.append(scroll)
        ...
        right=Gtk.Box(orientation=Gtk.Orientation.VERTICAL,spacing=8); right.set_margin_start(12); panes.set_end_child(right); self.group_header=Gtk.Label(label="Bir ders grubu seçin"); self.group_header.set_xalign(0); self.group_header.add_css_class("title-3"); right.append(self.group_header)
        self.lesson_list=Gtk.ListBox(); self.lesson_list.set_selection_mode(Gtk.SelectionMode.SINGLE); self.lesson_list.set_show_separators(True); self.lesson_list.set_vexpand(True); self.lesson_list.connect("row-selected",self._lesson_selected); lesson_scroll=Gtk.ScrolledWindow(); lesson_scroll.set_vexpand(True); lesson_scroll.set_child(self.lesson_list); right.append(lesson_scroll)
