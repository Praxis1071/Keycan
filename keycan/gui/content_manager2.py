"""User lesson group editor used by Keycan settings."""
...
class ContentManagerWindow(Adw.Window):
    ...
    def _clear(list_box):
        row = list_box.get_first_child()
        while row:
            nxt = row.get_next_sibling(); row.unparent(); row = nxt
    ...
    def _build(self):
        ...
        toolbar.set_content(root); self.set_content(toolbar)
        ...
        label = Gtk.Label(label="Ders gruplarım"); label.set_xalign(0); label.add_css_class("title-3"); left.append(label)
        ...
        group_create = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6); self.group_entry = Gtk.Entry(); self.group_entry.set_placeholder_text("Yeni ders grubu adı"); self.group_entry.set_hexpand(True); group_create.append(self.group_entry); b=Gtk.Button(label="Oluştur"); b.connect("clicked",self._create_group); group_create.append(b); left.append(group_create)
        ...
        editor=Adw.PreferencesGroup(); editor.set_title("Seçili metin"); editor.set_description("Metinlerin numarası sıralarına göre otomatik belirlenir."); right.append(editor)
    ...
    def _refresh_groups(self):
        ...
        for group_id,name,_key in self.db.custom_groups():
            row=Gtk.ListBoxRow(); row.group_id=group_id; l=Gtk.Label(label=name); l.set_xalign(0); l.set_margin_top(10); l.set_margin_bottom(10); l.set_margin_start(10); l.set_margin_end(10); row.set_child(l); self.group_list.append(row)
    ...
    def _group_selected(self,_list,row):
        ...
        group=next((g for g in self.db.custom_groups() if g[0]==self.selected_group_id),None); self.group_header.set_text(group[1] if group else "Ders grubu"); self._refresh_lessons(); self._set_group_controls(True)
    def _refresh_lessons(self):
        ...
        for number,(lesson_id,_text,_key,_order) in enumerate(lessons,1):
    ...
    def _create_group(self,_b):
        ...
        self.group_entry.set_text(""); self._refresh_groups()
        for row in self._rows(self.group_list):
            if getattr(row,"group_id",None)==self.selected_group_id:self.group_list.select_row(row);break
        self._changed("Ders grubu oluşturuldu.")
    def _rename_group(self,_b):
        if self.selected_group_id is None:return
        try:self.db.rename_custom_group(self.selected_group_id,self.group_entry.get_text())
        except ValueError as exc:self._message(str(exc));return
    ...
    def _delete_group_finish(self,dialog,result,_data):
        ...
        try:self.db.delete_custom_group(self.selected_group_id)
        except ValueError as exc:self._message(str(exc));return
    ...
    def _add(self,_b):
        if self.selected_group_id is None:return
        try:lesson_id=self.db.create_custom_lesson(self.selected_group_id,self._text())
        except ValueError as exc:self._message(str(exc));return
        self.selected_lesson_id=lesson_id;self._refresh_lessons()
        for row in self._rows(self.lesson_list):
            if getattr(row,"lesson_id",None)==lesson_id:self.lesson_list.select_row(row);break
        self._changed("Metin eklendi.")
    def _save(self,_b):
        if self.selected_lesson_id is None:return
        try:self.db.update_custom_lesson(self.selected_lesson_id,self._text())
        except ValueError as exc:self._message(str(exc));return
        self._changed("Metin kaydedildi.")
    def _delete(self,_b):
        if self.selected_lesson_id is None:return
        dialog=Gtk.AlertDialog(message="Seçili metni silmek istiyor musun?",detail="Metin bu ders grubundan kaldırılır."); dialog.set_buttons(["İptal","Sil"]); dialog.set_default_button(0); dialog.set_cancel_button(0); dialog.choose(self,None,self._delete_finish,None)
    def _delete_finish(self,dialog,result,_data):
        try:response=dialog.choose_finish(result)
        except Exception:return
        if response!=1:return
        try:self.db.delete_custom_lesson(self.selected_lesson_id)
        except ValueError as exc:self._message(str(exc));return
        self.selected_lesson_id=None;self._refresh_lessons();self._changed("Metin silindi.")
    def _move(self,direction):
        if self.selected_lesson_id is None:return
        try:self.db.move_custom_lesson(self.selected_lesson_id,direction)
        except ValueError as exc:self._message(str(exc));return
        self._refresh_lessons()
        for row in self._rows(self.lesson_list):
            if getattr(row,"lesson_id",None)==self.selected_lesson_id:self.lesson_list.select_row(row);break
