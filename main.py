...
import keycan.data.database_compat  # noqa: F401 — installs validated import support.
...
class ConfiguredKeycanWindow(keycan_window.KeycanWindow):
    ...
    def _install_sidebar_navigation(self) -> None:
        ...
        workspace_row = self._make_navigation_row("Çalışma Alanı", "input-keyboard-symbolic", "workspace")
        statistics_row = self._make_navigation_row("İstatistikler", "utilities-system-monitor-symbolic", "statistics")
        settings_row = self._make_navigation_row("Ayarlar", "emblem-system-symbolic", "settings")
        navigation.append(workspace_row); navigation.append(statistics_row); navigation.append(settings_row); navigation.select_row(workspace_row)
        ...
        settings_page = SettingsPanel(self, on_content_changed=self._load_sources, on_statistics_changed=statistics_page.refresh); settings_page.set_hexpand(True); settings_page.set_vexpand(True); content_stack.add_named(settings_page, "settings")
        ...
        workspace_row.set_name("workspace"); statistics_row.set_name("statistics"); settings_row.set_name("settings")
        split_view.set_sidebar(navigation); split_view.set_content(content_stack); toolbar.set_content(split_view)
    ...
    def _make_navigation_row(title: str, icon_name: str, page_name: str) -> Gtk.ListBoxRow:
        ...
        box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10); icon = Gtk.Image.new_from_icon_name(icon_name); icon.set_pixel_size(18); box.append(icon); label = Gtk.Label(label=title); label.set_xalign(0); label.set_hexpand(True); box.append(label); row.set_child(box); return row
